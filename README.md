...
3. Once the data is display, immediately come to the project terminal and execute command `node index <DURATION>` where `DUARTION` can be `1D`, `1W`, `1M`, `3M` or `6M`
4. Possible combinations to execute the command can be `node index 1D` OR `node index 1W` OR `node index 1M` OR `node index 3M` OR `node index 6M`
